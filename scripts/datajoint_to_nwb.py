...
session_key = acquisition.Session.fetch('KEY')[-2]
...
if probe_insertion:
    ...
    electrode_group = nwbfile.create_electrode_group(
        name='electrode group 1',  # TODO: not in pipeline
        description = 'N/A',
        device = probe,
...
if photostim:
    ...
    nwbfile.add_stimulus(pynwb.ogen.OptogeneticSeries(
        name='_'.join(['photostim_on', photostim['session_time'].strftime('%Y-%m-%d_%H-%M-%S')]),  # TODO: change 'session_time' back to 'photostim_datetime'
        site=stim_site,
        unit = 'mW',
        resolution = 0.0,
        conversion = 1e-6,
