...
for fname in fnames:
    ...
    subject_info = {c: nwb['general']['subject'][c].value.decode('UTF-8')
                    for c in ('subject_id', 'subject_description', 'sex', 'species', 'weight', 'age', 'genotype')}
    
    # dob and sex
    subject_info['sex'] = subject_info['sex'][0].upper()
    dob_str = re.search('(?<=Date of birth:\s)(.*)', subject_info['subject_description'])
    if utilities.parse_prefix(dob_str.group()) is not None:
        subject_info['date_of_birth'] = utilities.parse_prefix(dob_str.group())
        
    # source and strain
    strain_str = re.search('(?<=Animal Strain:\s)(.*)', subject_info['subject_description'])  # extract the information related to animal strain
    if strain_str is not None:  # if found, search found string to find matched strain in db
        for s in subject.StrainAlias.fetch():
            ...
            if m is not None:
                ...
                break
    source_str = re.search('(?<=Animal source:\s)(.*)', subject_info['subject_description'])  # extract the information related to animal strain
    ...
    if subject_info not in subject.Subject:
        subject.Subject.insert1(subject_info)
    
    # ==================== session ====================
    # -- session_time
    session_time = utilities.parse_prefix(nwb['session_start_time'].value)
    if session_time is not None and dict(subject_info, session_time=session_time) not in acquisition.Session:
        session_info = dict(
        ...
        reference.Experimenter.insert1(({'experimenter': k} for k in experimenters
                                        if {'experimenter': k} not in reference.Experimenter))
        reference.Experimenter.insert1(({'experiment_type': k} for k in experiment_types
                                        if {'experiment_type': k} not in reference.ExperimentType))

        with acquisition.Session.connection.transaction:
            acquisition.Session.insert1({**subject_info, **session_info})
            acquisition.Session.Experimenter.insert1(({'experimenter': k} for k in experimenters))
            acquisition.Session.ExperimentType.insert1(({'experiment_type': k} for k in experiment_types))
    ...
    trial_details = dict(trial_names=(t for t in nwb['epochs']),
                         trial_descs=(t for t in nwb['epochs'][t]['description'].value),
                         start_times=(t for t in nwb['epochs'][t]['start_time'].value),
                         stop_times=(t for t in nwb['epochs'][t]['stop_time'].value),
    ...
    if trial_key not in acquisition.TrialSet:
        ...
        for idx, trial_id in enumerate(trial_details['trial_names']):
            ...
            # -- events timing
            acquisition.TrialSet.EventTime.insert1(dict(trial_key, trial_event='trial_start', event_time = start_times[idx]),
                                                   ignore_extra_fields=True, skip_duplicates=True, allow_direct_insert=True)
            acquisition.TrialSet.EventTime.insert1(dict(trial_key, trial_event='trial_stop', event_time = stop_times[idx]),
                                                   ignore_extra_fields=True, skip_duplicates=True, allow_direct_insert=True)
            acquisition.TrialSet.EventTime.insert1(dict(trial_key, trial_event='cue_start', event_time = cue_start_times[idx]),
                                                   ignore_extra_fields=True, skip_duplicates=True, allow_direct_insert=True)
            acquisition.TrialSet.EventTime.insert1(dict(trial_key, trial_event='cue_end', event_time = cue_end_times[idx]),
                                                   ignore_extra_fields=True, skip_duplicates=True, allow_direct_insert=True)
            acquisition.TrialSet.EventTime.insert1(dict(trial_key, trial_event='pole_in', event_time = pole_in_times[idx]),
                                                   ignore_extra_fields=True, skip_duplicates=True, allow_direct_insert=True)
            acquisition.TrialSet.EventTime.insert1(dict(trial_key, trial_event='pole_out', event_time = pole_out_times[idx]),
                                                   ignore_extra_fields=True, skip_duplicates=True, allow_direct_insert=True)
    ...
    # -- BrainLocation
    if brain_location not in reference.BrainLocation:
    ...
    cell_key = dict({**session_info, **action_location},
                                  cell_id=cell_id,
                                  cell_type='N/A',
                                  device_name=ie_device)
    if cell_key not in acquisition.Cell:
        acquisition.Cell.insert1(cell_key)
    ...
    if f'device_name = {stim_device}' not in reference.PhotoStimDevice:
        reference.PhotoStimDevice.insert1({'device_name': stim_device, 'device_desc': devices[stim_device]})

    # -- PhotoStimulationInfo
    photim_stim_info = dict(action_location,
                            device_name=stim_device,
                            hoto_stim_excitation_lambda=float(opto_excitation_lambda),
                            photo_stim_notes=(f'{opto_site_name} - {opto_descs}'))
    if photim_stim_info not in stimulation.PhotoStimulationInfo:
    ...
    acquisition.PhotoStimulation.insert1(dict({**session_info, **photim_stim_info},
                                              photostim_timeseries=photostim_data,
                                              photostim_start_time=photostim_timestamps[0],
                                              photostim_sampling_rate=1/np.mean(np.diff(photostim_timestamps))))
